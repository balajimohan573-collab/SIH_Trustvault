...
import { speak, useLang, tr } from '../i18n'
...
export function SpeakerButton({ text, tone = 'slate' }: { text: string; tone?: Tone }) {
  const lang = useLang()
  return (
    <button
      type="button"
      aria-label={`${tr(lang, 'speak_button')}: ${text}`}
      title={`${tr(lang, 'speak_button')}: ${text}`}
